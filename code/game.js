...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (obstacle){
    ...
    if (keys.up && keys.left && this.speed.y > 0){
	this.speed.y = -jumpSpeed;}
	else if (keys.up && keys.right && this.speed.y > 0){
		this.speed.y = -jumpSpeed;
	}
    else
      this.speed.y = 0;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
	}else if (type == "saw" && this.status == null){
	this.status = "sawLost";
	this.finishDelay = 1;
	} else if (type == "coin") {
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost" || status == "sawLost"){
		  alert('Get back up Bearded Warrior, eat those pizzas!');
	  startLevel(n);}
	  else if (n < plans.length - 1){
		alert('Well done those pizzas must be delicious. Keep going!');
		startLevel(n + 1);}
	if (n == 4){
		  alert("There is another letter. 'Well done! You have beaten all of the challenges. So ya, congratulations. Also you owe me like $800.00 for those pizzas, I take check, credit, or cash! It was fun, chow!' Man this game was a rip off.")
		  startLevel(0)
	  }
      else
        console.log("You win!");
  ...
  startLevel(0);
  alert("I have know idea where I am. All I remember is sitting in front of my computer trying to finish some project for my digital design class. Also, where are my clothes!? Huh, what's this a letter? 'Welcome bearded warrior. You have been selected to take on a series of physical challenges. Your goal is simple: eat all the pizzas and survive. Don't touch the circular saws or pools of grease, else you'll be burnt to a crisp or worse you'll lose your majestic hair and beard! Use the left and right arrow keys to move around. Press the up arrow key to jump. The most important skill you have is the ability to run up, slide down, or hold onto walls. It is simple just push the left or right arrow key towards the wall to hold onto it. While holding the left or right arrow key push up to run up the wall or let go of the left or right arrow key to fall down. You cannot hold yourself in the same spot forever you will slowly slide down so be careful. There are more advanced techniques like wall jumping, but I think it is more fun for you to figure that out! Good luck and enjoy a slice for me. Sincerely, a Beard Enthusiasts. P.S. If you miss a pizza don't worry every level can be traversed in any direction. When you do eat all the pizzas you turn into a green, mutated beast!' Man what a chore, I guess I'll use this area to practice, because just like a video game, what type of challenge starts off hard?")
}
